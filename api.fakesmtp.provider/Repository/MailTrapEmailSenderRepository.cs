...
{
    public class MailTrapEmailSenderRepository : IMailTrapEmailSenderRepository
    {
        private MailTrapConfigurations _mailTrapConfigurations;
        public MailTrapEmailSenderRepository(MailTrapConfigurations mailTrapConfigurations)
        {
            _mailTrapConfigurations = mailTrapConfigurations;
        }
        public async Task<bool> SendingAttachmentEmailOfMailTrap(SendingAttachmentFileOfModel sendingAttachmentFileOfMailTrapModel)
        {
            var email = new MimeMessage();
            email.Sender = MailboxAddress.Parse(_mailTrapConfigurations.userName);
            email.To.Add(MailboxAddress.Parse(sendingAttachmentFileOfMailTrapModel.toEmailAddress));
            email.Subject = sendingAttachmentFileOfMailTrapModel.subject;

            BodyBuilder builder = new BodyBuilder();
            builder.TextBody = sendingAttachmentFileOfMailTrapModel.message;


            //Email Attachment
            if (sendingAttachmentFileOfMailTrapModel.emailAttachment != null)
            {
                using (MemoryStream memoryStream = new MemoryStream())
                {
                    await sendingAttachmentFileOfMailTrapModel.emailAttachment.CopyToAsync(memoryStream);
                    builder.Attachments.Add(sendingAttachmentFileOfMailTrapModel.emailAttachment.FileName, memoryStream.ToArray());
                }
            }
            email.Body = builder.ToMessageBody();

            using var smtp = new SmtpClient();
            smtp.Connect(_mailTrapConfigurations.host, _mailTrapConfigurations.port,SecureSocketOptions.StartTls);
            smtp.Authenticate(_mailTrapConfigurations.userName, _mailTrapConfigurations.password);
        ...
        public async Task<bool> SendingEmailOfMailTrap(SendingEmailOfModel sendingEmailOfMailTrapModel)
        {
            var email = new MimeMessage();

            email.Sender = MailboxAddress.Parse(_mailTrapConfigurations.name);
            email.To.Add(MailboxAddress.Parse(sendingEmailOfMailTrapModel.toEmailAddress));
            email.Subject = sendingEmailOfMailTrapModel.subject;

            BodyBuilder builder = new BodyBuilder();
            builder.TextBody = sendingEmailOfMailTrapModel.message;
            email.Body = builder.ToMessageBody();
        ...
        public async Task<bool> SendingHtmlEmailOfMailTrap(SendingEmailOfModel sendingEmailOfMailTrapModel)
        {
            var email = new MimeMessage();
            email.Sender = MailboxAddress.Parse(_mailTrapConfigurations.userName);
            email.To.Add(MailboxAddress.Parse(sendingEmailOfMailTrapModel.toEmailAddress));
